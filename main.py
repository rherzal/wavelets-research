# THIS FILE IS DEPRECATED AND HAS BEED SPLIT INTO DIFFERENT MODULES THAT HANDLE EACH EXPERIMENT
...
joint_data, emg_data, sampling_rate, time = interpolate_measurements(load_measurements(), sampling_rate)
...
# plt.plot(time[49:400], Y1[:, channel2print])
...
# X, Y = generate_multiple_position_regressors_only_emg(emg_data=emg_data, position_data=position, window_length=50, initial_time=0, final_time=350, level=levels, back_time=back_time)
...
# X1, Y2 = generate_multiple_position_regressors_only_emg(emg_data=emg_data, position_data=position, window_length=50, initial_time=350, final_time=480, level=levels, back_time=back_time)
# Y2 = reg.predict(X1)
